...
export default function TeachersPage() {
  ...
  // HÚT DỮ LIỆU TỪ GOOGLE SHEETS KHI MỞ TRANG
  useEffect(() => {
    // THẦY DÁN LINK GOOGLE SHEETS (.TSV) VÀO DƯỚI ĐÂY:
    ...
    fetch(sheetUrl)
      ...
      .then(text => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const cols = lines[i].split('\t'); // Tách cột bằng phím Tab
          data.push({
            slug: cols[0]?.trim(),
            name: cols[1]?.trim(),
            role: cols[2]?.trim(),
            image: cols[3]?.trim(),
            slogan: cols[4]?.trim(),
            stats: { courses: cols[5]?.trim(), students: cols[6]?.trim() },
            // Tách các giá trị có dấu phẩy thành mảng
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-8 relative z-10">
        ...
        {/* HIỆN CHỮ ĐANG TẢI KHI CHỜ DỮ LIỆU TỪ GOOGLE SHEET */}
        {loading ? (
